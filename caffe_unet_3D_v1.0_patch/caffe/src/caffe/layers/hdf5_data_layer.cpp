...
void HDF5DataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  DLOG(INFO) << "LoadHDF5FileData: Loading HDF5 file: " << filename;
...
void HDF5DataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int fi = 0; fi < num_files_; ++fi) {
    ...
    LOG(INFO) << "HDF5DataLayer:LayerSetUp: load HDF5 file: " << filename << std::endl;
    hid_t file_id = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    std::cout << "..opened." << std::endl;
    msg += filename + ":";
    for (int di = 0; di < num_datasets; ++di) {
      LOG(INFO) << "   dataset = " << this->layer_param_.top(di).c_str() << std::endl;
